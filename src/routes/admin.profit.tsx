...
export const Route = createFileRoute("/admin/profit")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Track profit per completed order, daily/weekly/monthly totals and product analytics.",
      },
      { property: "og:title", content: "Profit Dashboard — NexasStore Admin" },
      {
...
function ProfitDashboard() {
  ...
  const filtered = useMemo(() => {
    let rows = profits;
    if (range === "today") rows = rows.filter((r) => r.completedDate >= startOfToday());
    else if (range === "week") rows = rows.filter((r) => r.completedDate >= startOfWeek());
    else if (range === "month") rows = rows.filter((r) => r.completedDate >= startOfMonth());
    else if (range === "custom") {
      const start = from ? new Date(from + "T00:00:00").getTime() : 0;
      const end = to ? new Date(to + "T23:59:59").getTime() : Infinity;
      rows = rows.filter((r) => r.completedDate >= start && r.completedDate <= end);
    }
  ...
  const stats = useMemo(() => computeStats(profits), [profits]);
  const byProduct = useMemo(() => aggregateByProduct(filtered), [filtered]);

  const topSelling = useMemo(
    () => [...byProduct].sort((a, b) => b.units - a.units).slice(0, 5),
    [byProduct],
  );
  const highestProfit = useMemo(
    () => [...byProduct].sort((a, b) => b.profit - a.profit).slice(0, 5),
    [byProduct],
  );
  const lowestProfit = useMemo(
    () => [...byProduct].sort((a, b) => a.profit - b.profit).slice(0, 5),
    [byProduct],
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold">
            Profit <span className="text-gold-gradient">Dashboard</span>
          </h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={() =>
              downloadCsv(
                `nexas-profit-${new Date().toISOString().slice(0, 10)}.csv`,
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard label="Total Profit" value={rs(stats.total)} accent />
        <StatCard label="Today's Profit" value={rs(stats.today)} />
        <StatCard label="This Week's Profit" value={rs(stats.week)} />
        <StatCard label="This Month's Profit" value={rs(stats.month)} />
        <StatCard label="Total Completed Orders" value={String(stats.count)} />
        <StatCard label="Average Profit / Order" value={rs(stats.average)} />
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {(
            ...
            <Button
              key={key}
              size="sm"
              variant={range === key ? "gold" : "goldOutline"}
        ...
        {range === "custom" && (
          <div className="flex flex-wrap items-end gap-3">
            <div className="space-y-1.5">
              ...
              <Input id="pf-from" type="date" value={from} onChange={(e) => setFrom(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="pf-to">To</Label>
              <Input id="pf-to" type="date" value={to} onChange={(e) => setTo(e.target.value)} />
            </div>
          </div>
        )}
      </div>
      ...
      {/* History */}
      <h2 className="mt-8 font-display text-xl font-semibold">Profit History</h2>
      {loading ? (
        <div className="flex justify-center py-16">
          <Loader2 className="size-8 animate-spin text-primary" />
        </div>
      ) : filtered.length === 0 ? (
        <p className="py-16 text-center text-muted-foreground">No profit records for this range.</p>
      ) : (
      ...
      {/* Analytics */}
      <h2 className="mt-10 font-display text-xl font-semibold">Product Analytics</h2>
...
function StatCard({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-xl border border-border/60 bg-card p-4">
      ...
      <p
        className={`mt-1 font-display text-2xl font-bold ${accent ? "text-gold-gradient" : "text-primary"}`}
      >
        {value}
      </p>
    </div>
...
function ProfitRow({ row }: { row: ProfitRecord }) {
  ...
  return (
    <tr className="border-b border-border/40 last:border-0">
      ...
      <td className="px-4 py-3">{rs(row.salePrice)}</td>
      <td className="px-4 py-3 font-semibold text-primary">{rs(row.profitAmount)}</td>
...
function AnalyticsCard({
  ...
  rows: { name: string; image?: string; value: string }[];
}) {
  return (
    <div className="rounded-xl border border-border/60 bg-card p-4">
        ...
        <ul className="mt-3 space-y-2">
          {rows.map((r, i) => (
            <li key={i} className="flex items-center gap-2 text-sm">
              ...
              <span className="shrink-0 font-medium text-primary">{r.value}</span>
            </li>
          ))}
        </ul>
      )}
